...
function doSubmitApplication() { 
	global $mydb;   
        ...
        //trial above codenot permanent
		if (($picture=="")&&($front)&&($back)) {
			# code...
			...
			if (isset($_SESSION['APPLICANTID'])) {
				...
				$sql = "INSERT INTO `tblattachmentfile` (FILEID,`USERATTACHMENTID`, `FILE_NAME`, `FILE_LOCATION`, `JOBID`, `IDFRONT`, `IDBACK`) 
				VALUES ('". date('Y').$fileid->AUTO."','{$_SESSION['APPLICANTID']}','Resume','{$location}','{$jobid}','{$idfront}','{$idback}')";
				$mydb->setQuery($sql); 
				...
				$sql = "INSERT INTO `tblattachmentfile` (FILEID,`USERATTACHMENTID`, `FILE_NAME`, `FILE_LOCATION`, `JOBID`, `IDFRONT`, `IDBACK`) 
				VALUES ('". date('Y').$fileid->AUTO."','". date('Y').$applicantid->AUTO."','Resume','{$location}','{$jobid}','{$idfront}','{$idback}')";
				// echo $sql;exit;
				$mydb->setQuery($sql); 
				$res = $mydb->executeQuery(); 
...
//trial below
function UploadID($jobid=0){
	$target_dir = "applicant/photos/";
	$target_file = $target_dir . date("dmYhis") . basename($_FILES["front"]["name"]);
	$uploadOk = 1;
	$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
...
|| $imageFileType != "gif" ) {
		 if (move_uploaded_file($_FILES["front"]["tmp_name"], $target_file)) {
			return  date("dmYhis") . basename($_FILES["front"]["name"]);
		}else{
			message("Error Uploading File","error");
			// redirect(web_root."index.php?q=apply&job=".$jobid."&view=personalinfo");
			// exit;
...
function UploadIdback($jobid=0){
	$target_dir = "applicant/photos/";
	$target_file = $target_dir . date("dmYhis") . basename($_FILES["back"]["name"]);
	$uploadOk = 1;
	$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
...
|| $imageFileType != "gif" ) {
		 if (move_uploaded_file($_FILES["back"]["tmp_name"], $target_file)) {
			return  date("dmYhis") . basename($_FILES["back"]["name"]);
		}else{
			message("Error Uploading File","error");
			// redirect(web_root."index.php?q=apply&job=".$jobid."&view=personalinfo");
			// exit;
